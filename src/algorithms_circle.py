...
def adaptive_preprocessing(image:np.ndarray, brightness_mode="auto"):
    ...
    if gray.dtype != np.uint8:
        cv2.normalize(gray.astype(np.float32), gray, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
...
def _fit_circle_least_squares(points) -> None | Circle:
    if len(points) < 3:
        return None
    ...
    try:
        ...
        return Circle(float(cx), float(cy), r)
    except Exception:
        return None

def _fit_circle_ransac(points: np.ndarray, iterations=120, threshold=2.0, min_inliers=40) -> None | Circle:
    if len(points) < 3:
        return None
    best_circle: None | Circle = None
    best_inliers = 0
    N = len(points)
...
def _edge_points_outer_rim(gray: np.ndarray, prev_circle: Circle | None = None) -> None | np.ndarray:
...
def detect_circle_robust(gray:np.ndarray, prev_circle: Circle|None=None) -> Circle | None:
    ...
    if prev_circle is not None and span < (2*np.pi/3.0):  # <120°
        cand = Circle(cx, cy, prev_circle.radius)
    return cand
...
def _rough_center_radius(gray:np.ndarray, min_r, max_r) -> Circle | None:
    ...
    bw = cv2.morphologyEx(src=bw, op=cv2.MORPH_OPEN,
                          kernel=cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5)), iterations=1)
...
def _ring_mask(h, w, circle: Circle, inner=0.70, outer=1.15) -> np.ndarray:
    Y, X = np.ogrid[:h, :w]
    dist = np.sqrt((X - circle.x) ** 2 + (Y - circle.y) ** 2)
    m = ((dist >= circle.radius * inner) & (dist <= circle.radius * outer)).astype(np.uint8) * 255
    return m
...
def build_analysis_mask(img_gray: np.ndarray, brightness_min=3/255.0, min_radius=None, max_radius=None):
    """
    仅供 UI 调参窗口显示“分析区域”用：
    - uint8 归一化 -> 轻度去噪
    - Otsu 阈值 与 亮度下限并联
    ...
    try:
        g = img_gray.copy()
        if g.dtype != np.uint8:
            cv2.normalize(g.astype(np.float32), g, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
...
def detect_circle_phd2_enhanced(image: np.ndarray, hough: Hough, strong_denoise=False, prev_circle: Circle|None=None):
    """
    ...
    try:
        ...
        # —— 粗估中心半径，构建环形 ROI —— #
        est: Circle | None = _rough_center_radius(processed_det, hough.min_radius, hough.max_radius)
        if est is not None:
        ...
        if prev_circle is not None and all(np.isfinite(prev_circle)):
            try:
                ...
                inner = max(0.70, min(0.85, (hough.min_radius/max(pr,1e-6)) * 0.9))
                outer = min(1.30, max(1.15, (hough.max_radius/max(pr,1e-6)) * 1.05))
            ...
            if max_side > 1800:
                ...
                try:
                    sc = cv2.HoughCircles(
                        ...
                        param1=hough.param1, param2=max(hough.param2 - 5, 10),
                        minRadius=s_min, maxRadius=s_max,
                    )
                    if sc is not None:
                        c = sc[0][0]
                        circle = Circle(c[0] / scale, c[1] / scale, c[2] / scale)
        ...
        if time.time() - t0 > TIME_BUDGET:
            ...
            sc = cv2.HoughCircles(small, cv2.HOUGH_GRADIENT, dp=1.2, minDist=small.shape[0]//2,
                                  param1=max(hough.param1, 20), param2=max(hough.param2-5, 8),
                                  minRadius=max(1, int(hough.min_radius*scale)), maxRadius=max(2, int(hough.max_radius*scale)))
            if sc is not None:
                c = sc[0][0]
                best_circle = Circle(c[0]/scale, c[1]/scale, c[2]/scale)
        ...
        try:
            ...
            circles = cv2.HoughCircles(
                ...
                minRadius=hough.min_radius, maxRadius=hough.max_radius,
            )
        ...
        if need_pad:
            ...
            for c_p in circles_list:
                ...
                if q_adj > best_score:
                    # 映射回原图坐标
                    best_circle = Circle(cxp - pad, cyp - pad, rp)
                    best_score = q_adj
